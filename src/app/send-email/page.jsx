    ...
    <div>
      <div className="flex-1 flex flex-col">
        <main className="w-full flex justify-center pb-[7.5rem] py-[2.5rem] mx-auto md:my-[3.125rem] px-8 md:pb-0 md:max-w-4xl">
            <div className='flex flex-col'>
                <div className='flex flex-col md:flex-row items-center gap-4'>
                    ...
                    <span className="" style={{ opacity: 1, transform: "none" }}>
                        ...
                        <h1 className=" text-2xl md:text-4xl font-semibold leading-10 text-pretty">Send an Email</h1>
                      ...
                      <form onSubmit={handleSubmit}  className='flex flex-col gap-4 h-full'>
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="flex flex-row justify-around p-2 gap-5 ">
                            ...
                            <input  name="name"
                            value={formData.name}
                            onChange={handleInputChange} placeholder='Name' type="text" className='placeholder:text-white/60 text-white pl-2 capitalize  outline-none w-full bg-[#262626] p-2 rounded-lg' />
                            
                            <input name="email"
                            value={formData.email}
                            onChange={handleInputChange} placeholder='Email' type="email" className='placeholder:text-white/60 text-white pl-2 outline-none w-full bg-[#262626] p-2 rounded-lg' />
                            </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            onChange={handleInputChange} placeholder='Subject' type="text" className='placeholder:text-white/60 text-white pl-2 w-full mx-2  outline-none bg-[#262626] p-2 rounded-lg'/>
                          </div>
                          
                        </div>
                        
                        <textarea value={formData.message}
                        onChange={handleInputChange} placeholder='Message' name="message" id="" className='placeholder:text-white/60 text-white pl-2  mx-2 outline-none bg-[#262626] p-2 rounded-lg'></textarea>
                        
                        <button type='submit' className={`bg-[#262626] text-white/60 hover:text-white p-2 
                          rounded-lg w-16 mx-2 hover:bg-colorCard border border-colorBorder hover:border-colorBorder 
                          transition-all duration-300 ${isFormValid() && !isSubmitting ? "" : "opacity-10 cursor-not-allowed text-opacity-0"}`}
                          disabled={!isFormValid() || isSubmitting }>Send</button>
                      </form>
            ...
            {statusMessage && <p className="text-white/60 mt-4 text-center">{statusMessage}</p>}
            </div>
        </main>
      </div>
    </div>
